...
while True:
    ...
    if opcion == "1":  # Agregar un helado
        ...
        if len(heladoos) == 0:  # Error: variable incorrecta
            ...
            print("\nLista de Helados:")
            for helado in heladoos:
                print(f"ID: {helado['id']}, Nombre: {helado['nombre']}, Descripción: {helado['descripcion']}, Precio: {helado['precioo']}")  # Error en claves del diccionario
        ...
        if id_eliminar.isdigit():
            ...
            for heladoo in heladoos:
                if heladoos["id"] == id_eliminar:
